...
export default function ShadowingLessonList() {
  const c = useTheme();
    ...
    return (
      <View style={[styles.center, { backgroundColor: c.background }]}>
        ...
        <Text style={{ color: c.textSecondary, textAlign: 'center' }}>
          Chưa có bài nào cho trình độ {user?.level}.
        </Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={{ backgroundColor: c.background }} contentContainerStyle={styles.container}>
      {lessons.map((l) => (
        <Pressable
          key={l.id}
          onPress={() => router.push({ pathname: '/(app)/practice/shadow/[id]', params: { id: l.id } })}
          style={({ pressed }) => [
            styles.card,
